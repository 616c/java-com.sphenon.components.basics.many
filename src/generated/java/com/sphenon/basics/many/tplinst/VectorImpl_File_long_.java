...
public class VectorImpl_File_long_
  implements Vector_File_long_,
             VectorOptimized<File>,
             Dumpable,
             ManagedResource
 {
    private java.util.Vector vector;

    protected VectorImpl_File_long_ (CallContext context) {
        vector = new java.util.Vector(4);
    }

    static public VectorImpl_File_long_ create (CallContext context) {
        return new VectorImpl_File_long_(context);
    }

    protected VectorImpl_File_long_ (CallContext context, java.util.Vector vector) {
        this.vector = vector;
    }

    static public VectorImpl_File_long_ create (CallContext context, java.util.Vector vector) {
        return new VectorImpl_File_long_(context, vector);
    }
    ...
    public File get          (CallContext context, long index) throws DoesNotExist {
        try {
            return (File) vector.elementAt((int) index);
    ...
    public VectorReferenceToMember_File_long_ getReference    (CallContext context, long index) throws DoesNotExist {
        ...
        return new VectorReferenceToMember_File_long_(context, this, index);
    }
    ...
    public File set          (CallContext context, long index, File item) {
        ...
        return (File) vector.set((int) index, item);
    }
    ...
    public void     prepend      (CallContext call_context, File item) {
        if (vector.size() == 0) {
            vector.add(item);
        } else {
            try {
    ...
    public File replace      (CallContext call_context, long index, File item) throws DoesNotExist {
        try {
            return (File) vector.set((int) index, item);
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    public File unset        (CallContext context, long index) {
        try {
            return (File) vector.remove((int) index);
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    public boolean contains(CallContext context, File item) {
        return this.vector.contains(item);
    }

    public boolean removeFirst(CallContext context, File item) {
        return this.vector.remove(item);
    }

    public void removeAll(CallContext context, File item, VectorOptimized.Notifier<File> notifier) {
