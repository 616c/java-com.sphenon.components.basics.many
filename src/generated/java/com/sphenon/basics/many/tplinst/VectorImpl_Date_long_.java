...
public class VectorImpl_Date_long_
  implements Vector_Date_long_,
             VectorOptimized<Date>,
             Dumpable,
             ManagedResource
 {
    private java.util.Vector vector;

    protected VectorImpl_Date_long_ (CallContext context) {
        vector = new java.util.Vector(4);
    }

    static public VectorImpl_Date_long_ create (CallContext context) {
        return new VectorImpl_Date_long_(context);
    }

    protected VectorImpl_Date_long_ (CallContext context, java.util.Vector vector) {
        this.vector = vector;
    }

    static public VectorImpl_Date_long_ create (CallContext context, java.util.Vector vector) {
        return new VectorImpl_Date_long_(context, vector);
    }
    ...
    public Date get          (CallContext context, long index) throws DoesNotExist {
        try {
            return (Date) vector.elementAt((int) index);
    ...
    public Date set          (CallContext context, long index, Date item) {
        ...
        return (Date) vector.set((int) index, item);
    }
    ...
    public void     prepend      (CallContext call_context, Date item) {
        if (vector.size() == 0) {
            vector.add(item);
        } else {
            try {
    ...
    public Date replace      (CallContext call_context, long index, Date item) throws DoesNotExist {
        try {
            return (Date) vector.set((int) index, item);
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    public Date unset        (CallContext context, long index) {
        try {
            return (Date) vector.remove((int) index);
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    public boolean contains(CallContext context, Date item) {
        return this.vector.contains(item);
    }

    public boolean removeFirst(CallContext context, Date item) {
        return this.vector.remove(item);
    }

    public void removeAll(CallContext context, Date item, VectorOptimized.Notifier<Date> notifier) {
